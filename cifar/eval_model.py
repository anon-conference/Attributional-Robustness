...
model = backbone.WideResNet28_10_r( flatten = True, beta_value = 50.)
checkpoint_dir = './checkpoints/%s/%s_%s_%s' %('cifar', 'WideResNet28_10_r', 'srt' ,  'cifar_saliency_b50')
...
def get_grad(img, y, model):
    ...
    g = torch.autograd.grad(loss, img, retain_graph=True, create_graph=True)[0]
    gradient = g.detach()
...
def get_gradIG(img ,y,model):
    ...
    integrated_grad = (inputs_sq - baseline) * avg_grads.cpu().numpy()
...
for i , (x,y) in enumerate(testloader):
    ...
    gg1 = get_grad(normalize(x_var),y,model)
    gg1_ig = get_gradIG(x_var ,y ,model)
        
    gg2 = get_grad(normalize(xadv),y,model)
    gg2_ig = get_gradIG(xadv ,y ,model)
   
    
    for each1 , each2 , x_each in zip(gg1 , gg2 , x_var):
        ...
        tau2 , v2 = stats.spearmanr( each1 , each2 )

        tau_values[0].append(tau1)
        tau_values[1].append(tau2)
        ...
        tau_values[2].append(intersection)
    ...
    for each1 , each2 , x_each in zip(gg1_ig , gg2_ig , x_var):
        ...
        tau_values[3].append(tau1)    
        tau_values[4].append(tau2)    
        ...
        perturbed_ig_topK = np.argsort(each2)[-eval_k_top:]
        intersection = float(len(np.intersect1d(origin_ig_topK, perturbed_ig_topK))) / eval_k_top
        tau_values[5].append(intersection)

    
    if i%10==0:
        print(count , correct/total , correct_adv/total)
        print("kendall_g" , np.mean(tau_values[0]))
        print("spearman_g" , np.mean(tau_values[1]))
        print("intersection_g" , np.mean(tau_values[2]))
        
        print("kendall_ig" , np.mean(tau_values[3]))
        print("spearman_ig" , np.mean(tau_values[4]))
        print("intersection_ig" , np.mean(tau_values[5]))
